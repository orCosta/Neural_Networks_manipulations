...
def get_z_values(size, type='uni'):
    if type == 'uni':
        z = np.random.uniform(0., 1., size=size)
        return z
    if type == 'gmm':
        means = np.random.randn(10) * 6
        vec = np.random.choice(means, size=size)
        z = (0.1 * np.random.randn(size[0], size[1])) + vec
        return z
    if type == 'mnist':
        data = np.load('mnist_latent_vec.npy')
        z = data[np.random.choice(data.shape[0], size[0], replace=False), :]
        return z
